...
public static class DxpComputedTableStyleCssExtensions
{
	...
	private static void AppendBoxBorderCss(StringBuilder sb, DxpComputedBoxBorders borders)
	{
		...
		if (borders.Top != null) AppendCssProperty(sb, "border-top", borders.Top.ToCssValue());
		if (borders.Right != null) AppendCssProperty(sb, "border-right", borders.Right.ToCssValue());
		if (borders.Bottom != null) AppendCssProperty(sb, "border-bottom", borders.Bottom.ToCssValue());
		if (borders.Left != null) AppendCssProperty(sb, "border-left", borders.Left.ToCssValue());
	}
