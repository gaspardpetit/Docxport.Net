...
internal static class DxpParagraphLayoutComputer
{
	public static DxpComputedParagraphLayout? ComputeLayout(Paragraph p, DxpDocumentContext d)
	{
		...
		if (tabs == null)
			return new DxpComputedParagraphLayout(Array.Empty<DxpComputedTabStop>());
	...
	{
		if (kind == TabStopValues.Right) return DxpComputedTabStopKind.Right;
		if (kind == TabStopValues.Center) return DxpComputedTabStopKind.Center;
		if (kind == TabStopValues.Decimal) return DxpComputedTabStopKind.Decimal;
		if (kind == TabStopValues.Left) return DxpComputedTabStopKind.Left;
		return null;
	}
}
